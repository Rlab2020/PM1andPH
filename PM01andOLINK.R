...
###--------------------------------------------------------------------
### LOADING DATA ###
load("/Users/guuuuu/Desktop/data_pm01_p.rda")
...
table4a <- matrix(NA, nrow = length(olinklist), ncol = 4*length(variables))
colnames(table4a) <- c("coef1","ci1","ci2","p1")
rownames(table4a) <- olinklist

for (i in seq_along(variables)) {
  variable_i <- variables[i]
  for (j in seq_along(olinklist)) {
    olinklist_j <- olinklist[j]
    table4a[j, (4 * i - 3):(4 * i)] <- FUN4(variable_i,olinklist_j,"data0")
    print(paste0("Done ",variables[i]," ",olinklist[j]))}}
...
library(writexl)
write_xlsx(table4a, path = "/Users/guuuuu/Desktop/TABLE - PM01-OLINK BY LINEAR.xlsx")


#######################################################################
###--------------------------------------------------------------------
### ELASTIC NET REGRESSION
table4a <- read_excel("/Users/guuuuu/Desktop/TABLE - PM01-OLINK BY LINEAR.xlsx")
...
olinklist1 <- table4a$name

X <- data0_training %>% select(all_of(olinklist1),all_of(covariates))
X_matrix <- makeX(X)
Y <- data0_training[["mean0610b1000"]]

set.seed(123)  
cv_model <- cv.glmnet(X_matrix, Y, alpha = 0.5, nfolds = 10, 
                      penalty.factor = ifelse(colnames(X_matrix) %in% olinklist1, 1, 0))
...
table4b <- table4b %>%
  ...
  filter(name %in% olinklist1) %>%
  rename(coef = s0) %>%
  select(name, coef)

library(writexl)
write_xlsx(table4b, path = "/Users/guuuuu/Desktop/TABLE - PM01-OLINK BY ENR.xlsx")
...
library(writexl)
write_xlsx(table4b, path = "/Users/guuuuu/Desktop/TABLE - PM01-OLINK BY LR.xlsx")
